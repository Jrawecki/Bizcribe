          ...
          {isAuthenticated ? (
            <>
              <span className="text-sm opacity-90">
                {user.display_name || user.email} <span className="opacity-70">· {role}</span>
              </span>
              <button onClick={logout} className="px-3 py-2 rounded-full btn-ghost">Logout</button>
            </>
          ) : (
            <>
              <Link to="/login" className="px-3 py-2 rounded-full btn-ghost">Login</Link>
              <Link to="/register" className="px-3 py-2 rounded-full btn-primary">Sign up</Link>
            </>
          )}
        </div>
      </div>
